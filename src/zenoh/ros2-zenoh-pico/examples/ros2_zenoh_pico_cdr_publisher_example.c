...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < 20; i++) {
        ...
        float angle = (float)i * 0.314159f; // ~18 degrees per step
        ...
        twist.angular_z = 0.5f * sinf(angle);  // Oscillating turn rate
        ...
        int result = ros2_zenoh_pico_publish(
            node,
            "ros2/turtle1/cmd_vel",  // Bridge topic
            cdr_buffer,
            serialized_size
        );
        ...
        ros2_zenoh_pico_sleep_ms(500);  // 500ms delay
    }
